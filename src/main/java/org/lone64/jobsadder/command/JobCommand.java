...
import org.lone64.jobsadder.inventory.InvJobSelector;
...
public class JobCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String arg, @NotNull String[] args) {
        ...
        if (JobsAdder.getInstance().getRepository().isJobPlayer(player)) {
            ...
            AdventureUtil.playerMessage(player, JobsAdder.getLangConfig().getString("already-selected-job"));
            return true;
        }

        new InvJobSelector(player).load();
        return false;
    }
